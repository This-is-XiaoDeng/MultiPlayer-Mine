...
def draw_map(game_map):
    ...
    if game_map != _map:
        logger.info("开始绘制地图 ...")
        # 初始化
        surface = pygame.surface.Surface((len(game_map)*BLOCK_SIZE, len(game_map[0])*BLOCK_SIZE))#(210, 210))
        ...
        for row in game_map:
            ...
            for column in row:
                if column >= 0:
                    pygame.draw.rect(surface, (255, 255, 255), (width, height, 18, 18))
                elif -9 <= column < 0:
...
def draw(game_map):
    ...
    surface_size = (map_size[0] * BLOCK_SIZE + 20, map_size[1] * BLOCK_SIZE + 100)
    surface = pygame.surface.Surface(surface_size)
    surface.fill((255, 255, 255))
    pygame.draw.rect(surface, (0, 0, 0), (5, 5, surface_size[0] - 10, surface_size[1] - 10))
    surface.set_colorkey((0, 0, 0))
    map_surface = draw_map(game_map)
    surface.blit(map_surface, (10, surface_size[1] - map_surface.get_size()[1] - 10))
